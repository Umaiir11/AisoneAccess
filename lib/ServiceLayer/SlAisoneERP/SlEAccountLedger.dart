...
class SlEAccountLedger {
  ...
  Future<List<ModI_AccountLedger>?> Fnc_AccountLedger(DateTime StartDate,DateTime EndDate ,) async {
    try {
      //" Where UserDID = '" & Pb_UserDID.ToString & "' and IsAssigned = 1"
      final DateFormat formatter = DateFormat('yyyy-MM-dd');
      final String? formatted = formatter.format( StartDate);
      final String? formatted2 = formatter.format(EndDate);
    String l_accountDID =    cmGlobalVariables.Pb_ListUserAccountsQuery[0].Pr_AccountsDID;

      final body = {
        "Pr_AccountDID": l_accountDID,
        "Pr_FromDate": formatted,
        "Pr_ToDate": formatted2
      };
  ...
  List<ModI_AccountLedger> Fnc_JsonToListOfModel(List<dynamic> l_JsonList) {
    List<ModI_AccountLedger> l_ListModI_AccountLedger =
